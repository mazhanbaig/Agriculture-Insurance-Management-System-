...
export async function listStaffUsers(page: number, limit: number, role?: string) {
  ...
  const where: any = { role: { not: "FARMER" } };
...
export async function getDashboardAggregates() {
  try { const cached = await redis.get(DASHBOARD_CACHE_KEY); if (cached) return JSON.parse(cached); } catch { /* skip cache */ }
  ...
  try { await redis.setex(DASHBOARD_CACHE_KEY, DASHBOARD_CACHE_TTL, JSON.stringify(dashboard)); } catch { /* non-critical */ }
  return dashboard;
}
